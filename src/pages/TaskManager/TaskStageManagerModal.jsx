...
import { createTaskStage, updateTaskStage, deleteTaskStage } from '../../lib/api/taskStages';
import { ColorPickerControl, DEFAULT_QUICK_COLORS, sameColor } from '../../components/common/ColorPickerControl';
...
// Task-stage analog of Deals' StageManagerModal.jsx — same "stage local,
// diff-on-save" mechanics, is_won/is_lost renamed is_done/is_cancelled,
// pipeline_id renamed department_id. Everything here is staged in local
// `rows` state and only actually sent to Supabase when "Готово" is clicked
// (diffed against the `stages` prop).
export default function TaskStageManagerModal({ departmentId, departmentName, stages, onClose, onChanged }) {
  ...
  // Only one stage can be the "done" result and only one can be the
  // "cancelled" result — checking one clears it off every other row, and
  // clears the opposite flag on this same row (a stage can't be both).
  function handleFlag(id, field, value) {
    const opposite = field === 'is_done' ? 'is_cancelled' : 'is_done';
    setRows((r) => r.map((s) => {
  ...
  function handleAddStage() {
    ...
    if (!label) return;
    setRows((r) => [...r, { id: newRowId(), label, color: newColor, is_done: false, is_cancelled: false }]);
    setNewLabel('');
  ...
  async function handleDone() {
    ...
    if (!rows.some((s) => s.is_done)) { alert('Потрібен рівно один етап «Виконано».'); return; }
    if (!rows.some((s) => s.is_cancelled)) { alert('Потрібен рівно один етап «Скасовано».'); return; }
    setSaving(true);
    const errors = [];
    ...
    for (const s of stages) {
      if (!currentIds.has(s.id)) {
        try { await deleteTaskStage(s.id); } catch (e) { errors.push(`«${s.label}»: ${e.message || e}`); }
      }
    }
    ...
    for (const s of rows) {
      ...
      try {
        const created = await createTaskStage({ label: s.label.trim(), color: s.color, departmentId });
        if (s.is_done || s.is_cancelled) await updateTaskStage(created.id, { is_done: !!s.is_done, is_cancelled: !!s.is_cancelled });
    ...
    for (const s of rows) {
      ...
      if (!!s.is_done !== !!orig.is_done) patch.is_done = !!s.is_done;
      if (!!s.is_cancelled !== !!orig.is_cancelled) patch.is_cancelled = !!s.is_cancelled;
      if (Object.keys(patch).length) {
        try { await updateTaskStage(s.id, patch); } catch (e) { errors.push(`«${s.label}»: ${e.message || e}`); }
      }
    }
    ...
    for (const s of rows) {
      ...
      if (!orig || orig.position !== idx) {
        try { await updateTaskStage(realId, { position: idx }); } catch (e) { errors.push(e.message || String(e)); }
      }
    }
      ...
      <div className="tmodal-box stage-manager-modal">
        <div className="tmodal-head">
          <div className="pipeline-modal-head">
            ...
            <div>
              <h3>Налаштувати етапи{departmentName ? ` — ${departmentName}` : ''}</h3>
        ...
        <div className="tmodal-body">
          <div className="stage-info-banner">
            ...
            <span>Можна мати лише один етап <b>«Виконано»</b> і один етап <b>«Скасовано»</b>. Ці етапи вважаються фінальними результатами задачі.</span>
          </div>
            ...
            {rows.map((s, i) => (
              ...
              >
                ...
                <div className="stage-final-cell">
                  <label className={'stage-final-check' + (s.is_done ? ' active-won' : '')}>
                    <input type="checkbox" checked={!!s.is_done} onChange={(e) => handleFlag(s.id, 'is_done', e.target.checked)} />
                    <span className="stage-final-box" dangerouslySetInnerHTML={{ __html: CHECK_ICON }} />
                    Виконано
                  </label>
                  <label className={'stage-final-check' + (s.is_cancelled ? ' active-lost' : '')}>
                    <input type="checkbox" checked={!!s.is_cancelled} onChange={(e) => handleFlag(s.id, 'is_cancelled', e.target.checked)} />
                    <span className="stage-final-box" dangerouslySetInnerHTML={{ __html: CHECK_ICON }} />
                    Скасовано
                  </label>
                </div>
